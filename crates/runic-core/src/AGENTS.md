...
- `Layout` stays at the `GlobalAlloc` / public `Allocator` boundary. Convert once with `LayoutSpec::from_layout` and pass `LayoutSpec` through classify (`SizeClasses::id_for`), extent allocate/reuse/resize, and run `resize_in_place`. Do not re-normalize size/align from raw `Layout` deeper in the stack, and do not add free layout helpers beside `LayoutSpec` methods.
- Allocator routing pairs owner-local TLS with remote table paths: small `ThreadHeap::alloc` / `Allocator::alloc_remote`, large `ThreadHeap::alloc_extent` / `Allocator::alloc_extent_remote`, free `ThreadHeap::free`/`free_extent` / `Allocator::free_remote`. Remote alloc means bind a heap via `HeapTable` then allocate there — not a second allocate protocol. Do not revive `*_slow` / `*_miss` / `*_nonlocal` path names; coldness is `#[cold]` only.
- `SizeClasses::id_for(LayoutSpec)` keeps default-align (`align <= 8`) as the hot fall-through: size bound + `CLASS_FOR_SIZE` only (no `PAGE_SIZE` probe on that path). Over-aligned / large-align remap stays in the same method (no cold one-line helper split). Do not reintroduce a second bounds check via `lower_bound_index` on the default-align path.
