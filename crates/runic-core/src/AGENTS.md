...
- `SizeClassId` only from `SizeClasses`; use `block_size(id)`; `block_offset` / `block_index_from_offset` for run payload indexing; const-generate align remap from `SIZES`.
