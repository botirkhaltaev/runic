...
- `ThreadHeap`: sole Active body path (`alloc` / `free_*` / `flush` / `alloc_*_after_bind`). Hit: current-run pop / page-cache `Run::free` (`push_available` only on `was_full`). `#[cold]` only abort / bind / map / remote / unbind.
