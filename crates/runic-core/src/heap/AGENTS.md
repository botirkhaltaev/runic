...
- Exclusive metadata is `Mutex<HeapInner>`. Active: `require_inner` (`try_inner` or abort), including an adopted heap; opportunistic adopted retirement returns when `try_inner` is busy. `adopt` flush is the one `lock_inner` wait for an in-flight Draining admit. Draining: `lock_inner` after mode check. `AllocatorCtx` is the parent bag, not a lock guard. `unbind` retires bound and adopted.
