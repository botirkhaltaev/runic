...
- `ThreadHeap`: `bind` / `unbind`, owner-local `alloc` / `free` / `dealloc` / `alloc_extent` / `free_extent`, `lookup_owner`, `batch` / `take_batch` — no allocate/dealloc routers mirrored from `HeapTable`.
...
- Empty sticky refill sets sticky `run_cell` in `acquire_alloc`; keep one-shot `Heap::alloc_run` off sticky under the table lock.
