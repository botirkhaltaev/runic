...
- Owner non-cached free is `Heap::free(PageOwner)` (may flush inbox); TLS sticky run free stays `Run::free` only (live ownership lives on the run; no `was_full` / `finish_free`; sticky free errors are `#[cold]`).
