...
- Keep `ThreadHeap` thin and composable: `bind` / `unbind`, owner-local `alloc` / `free`, `bound`, and remote `batch` / `take_batch`. Do not mirror `Heap`/`HeapTable` allocate-dealloc routers on TLS.
...
- Do not put `HeapTable` on steady-state owner-local allocation hot paths (`ThreadHeap::alloc` / cached `free` must not take the table mutex).
