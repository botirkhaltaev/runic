...
- Keep `ThreadHeap` thin and composable: `bind` / `unbind`, owner-local `alloc` / `free` (runs) and `alloc_extent` / `free_extent` (extents), `bound`, and remote `batch` / `take_batch`. Sticky hit is the straight-line body of `alloc` / `free`; miss paths are cold. Do not mirror `Heap`/`HeapTable` allocate-dealloc routers on TLS. Extents have no sticky TLS slot cache; mapping reuse stays on `ExtentCache`.
- `Allocator::alloc` / `dealloc` must keep run sticky shells free of in-line extent work: outline large alloc (`alloc_extent_hot`) and owner-local extent free (`dealloc_extent_hot`). Sharing those bodies in the small shells regresses `single_size_churn` codegen (TLS/`LocalKey::with` no longer folds into `Allocator::alloc`).
