...
- Owner free composition stays on `Heap` methods that take `&mut HeapInner` plus ctx; domain ops are `free` / `claim` / `accept` on `Run`/`Extent`. Failures after claim abort (no rollback).
- Current-run empty: `extend`; accept inbox if nonempty (same as `alloc_extent`); then local/OS `acquire_run`. Unbound cold path: `alloc_after_bind` / `alloc_extent_after_bind`. Hit: current pop / page-cache `Run::free`. Inbox `flush` is remote `accept`.
