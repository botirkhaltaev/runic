...
- Owner free composition stays on `Heap::free(PageOwner)` / `flush`; domain ops are `free` / `claim` / `accept` on `Run`/`Extent`. Allocator routing is `ThreadHeap::free_local` (one TLS entry) / `Allocator::free_remote`.
