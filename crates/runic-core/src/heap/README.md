...
- Cross-thread frees: Active publisher lease → `claim` → TLS batch → inbox `push_batch` (or `HeapDirectory::publish` on target change); Draining uses locked `free_draining` / accept. Owner `flush` → `accept`.
