...
- `ExtentHeap::has_live_extents` is occupied-arena presence (extents leave the arena on free); pair with `RunHeap::has_live_blocks` for Draining reclaim — no heap-level alloc side counter.
