...
- Owner Free/Live **authority** is freelist membership (+ bump). Allocate pops/bumps then `live++`; bump allocate stores no `BlockStates`; freelist allocate observes Free then `store(Clear)` (no CAS). Owner free and remote claim still CAS Clear↔Free / Clear↔RemotePending.
