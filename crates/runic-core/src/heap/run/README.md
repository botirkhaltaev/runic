...
- `Run::free` returns `Result<_, RunError>` (`InvalidPointer` only on the owner path). `accept` returns `bool` (needs re-push). `RunHeap` reads `is_full()` before those ops for available-list relinking.
