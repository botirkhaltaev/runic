...
- A run owns one size class and one range in a heap-owned map (not its own `Mapping`). The base is `RUN_SIZE`-aligned. Payload is `RUN_SIZE` bytes; the `Run` header lives at `base + RUN_SIZE` (`base`/`span`/`recip` next to `RunState`; remote `link`/`claims` on the next 64-byte line); claim words follow the header. `Run::range` is the payload span only. `PageMap::publish_run` stamps that payload. A map holds `MAP_RUNS` spaces (`RUN_SPACE` each). Small free uses `Run::header_of` (`ptr & !(RUN_SIZE-1)`); self-check `base`, else `PageMap`.
...
- Live small ownership for reclaim is `Run::is_live` (allocated or remote-claimed), aggregated by `RunHeap::has_live` over occupied in-space headers.
