...
- Freelist allocate trusts internal freelist/bump bases; frees of user pointers validate block boundaries via `block_at`.
- `BlockStates` is the only free / allocated / remote-pending tracker (one `AtomicU8` per block for the run's capacity, viewed as an `AddressRange` over the mapping's state tail).
...
- Live small ownership for reclaim is `Run::has_live_blocks` over occupied arena runs (allocated + remote-pending).
