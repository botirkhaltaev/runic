...
- `BlockStates` is one `AtomicU8` per block for this run's capacity, stored in the run mapping after the `RUN_SIZE` payload span — not a packed bitmap and not a worst-case inline array. It is the only free/allocated/remote-pending tracker; do not add a second freelist or bitset.
