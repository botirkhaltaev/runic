...
- Domain ops on `Run`: `allocate` (refill) / `free` (`Claimed` if claim won) / `claim` / `accept` (free = store+recheck; claim = set-bit+recheck; `accept` clears queued then drains claim words).
