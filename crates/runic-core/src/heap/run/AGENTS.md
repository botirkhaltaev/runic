...
- `BlockStates` is one `AtomicU8` per block for this run's capacity, owned in a dedicated anonymous mapping — not a packed bitmap and not a worst-case inline array. It is the only free/allocated/remote-pending tracker; do not add a second freelist or bitset.
