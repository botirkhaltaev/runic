...
- Domain ops on `Run`: `allocate` / `extend` / `free` (locate + push) / `claim` / `accept`. Hit ignores `was_full` / Discard. `discard_empty` is miss / accept. Owner DF undefined.
- Embedded `InboxLink` coalesces by run (one inbox entry for many claims).
- `RunHeap::acquire` (available or take/map) backs `acquire_run`. Available-list: at most once; `push_available` is idempotent; current may be listed; unbind returns current. Occupied-run count is `RunHeap` live (0↔1 on `allocate` / `free` / `accept`).
