...
- `page_map/`: page-indexed lookup from user pointers to `PageOwner` metadata pointers.
  ...
  - `table.rs`: `L1Table` (hot `tables` + cold `meta`), `L1WriteGuard`, `L1Meta`, `L2Table` — install, RAII per-L2 write exclusion, owner lookup, segment match/write.
...
- L1 root split: `get` indexes only the dense `tables` pointer array; stamp/install cold state lives in parallel `meta`. L1/L2 mmap ownership is stored by the install CAS winner; table pointers are published atomically; L2 tables stay for the `PageMap` lifetime. Stamp writers take per-L2 exclusion on `meta`; `get` never reads meta/mapping cells or takes write locks.
...
- No further L1 densify beyond hot/cold split: the table spans the full 48-bit address space and depends on OS lazy paging; VA shape not revisited without profiling data.
