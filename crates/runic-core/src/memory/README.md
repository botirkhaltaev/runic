...
- `page_map/`: page-indexed lookup from user pointers to `PageOwner` metadata pointers.
  - `mod.rs`: `PageMap` API (`publish_run`, `publish_extent`/`unpublish_extent`, `get`) — publish takes `&Mapping`; once-only L1/L2 CAS install; stamps under `L1WriteGuard`; lock-free `get` walks L1 tip → slot tip → page entry.
  ...
  - `table.rs`: `L1Table` (hot `L1Slot` array + cold `mappings`), `L1WriteGuard`, `L2Table` — install, RAII per-L2 write exclusion on the hot slot, owner lookup, segment match/write.
...
- L1 root split: hot `L1Slot` holds tip + write exclusion (16-byte stride); L2 mmap ownership lives in parallel `mappings`. `get` loads only the tip; stamp writers take per-L2 exclusion on the slot; `get` never reads mapping cells.
...
- No further L1 densify beyond hot slots + cold mappings: the table spans the full 48-bit address space and depends on OS lazy paging; VA shape not revisited without profiling data.
