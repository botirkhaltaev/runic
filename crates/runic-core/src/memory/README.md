...
- Small free / realloc probes `Run::header_of` first (in-page header at `base + RUN_SIZE`, raw base self-check before constructing a `Run` pointer). `PageMap::get` is miss, extent, and fallback. `publish_run` stamps run payload pages only.
