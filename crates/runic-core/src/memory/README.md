...
- `page_map/`: page-indexed lookup from user pointers to `PageOwner` metadata pointers.
  - `mod.rs`: `PageMap` API (`publish_run`, `publish_extent`/`unpublish_extent`, `get`) — publish takes `&Mapping`; once-only L1/L2 CAS install; stamps under `L1WriteGuard` (per-L2 write exclusion) with `store`; `get` is lock-free Acquire loads.
  ...
  - `table.rs`: `L1Table` / `L1WriteGuard` / `L1Entry` / `L2Table` — L2 install, RAII per-L2 `AtomicBool` write exclusion, segment match/write.
