...
- Every `Mapping` from `OsMemory::map` / `map_aligned` only. `Mapping` Drop munmaps; do not munmap by hand. `OsMemory::discard` is `madvise(MADV_DONTNEED)` and ignores failure.
