...
- Multi-page insert/remove: install L2s, lock touched `L1Entry`s in ascending L1 order, validate under those locks, store, unlock. Failed validate unlocks with no writes (map unchanged).
...
- Put page-map behavior on the owning type: `AtomicMapEntry` (load/store), `L2Table` (segment match/write under caller-held L2 write lock), `L1Entry` (L2 install + write exclusion), `L1Table` (ordered lock/stamp for a range), `PageMap` (publish/get composition).
