...
- `PageMap` writers are lock-free: once-only L1/L2 install via `AtomicPtr` CAS, per-page stamps via `AtomicMapEntry::compare_exchange`. Do not reintroduce a write-path mutex (including an empty `Mutex<()>`), occupancy counters, or unconditional `store` as a second mutation API.
...
- Multi-page insert/remove is all-or-nothing via reverse-CAS on failure. If a remove restore CAS fails (concurrent writer on the same VA), call `Allocator::abort`.
...
- Put page-map behavior on the owning type: `AtomicMapEntry` (CAS), `L2Table` (segment assign/clear), `L1Entry` (L2 install + forward), `PageMap` (range composition). Do not add lock-era validate-then-write dual paths.
