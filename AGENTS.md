...
- Flush policy: current-run empty = `extend` then local/OS `acquire_run` first, inbox flush only on miss; unbound = `alloc_after_bind` / `alloc_extent_after_bind` (flush then alloc); hit = current pop / page-cache `Run::free` (`push_available` only on `was_full`). Inbox `flush` is remote `accept` only.
