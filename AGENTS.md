...
- One remote-free protocol: claim → batch → `HeapDirectory::publish` (Active lease / Draining locked accept) → flush/`accept`. Never-bound freers publish each claim in `free_remote` so Drop cannot strand `RemotePending`.
