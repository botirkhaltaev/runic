...
- Flush policy: current-run empty = `extend`; inbox flush if nonempty; then local/OS `acquire_run`. Unbound = `bind` then `flush` then alloc; hit = current pop / `Run::release`. `push_available` is miss / slow / unbind. Inbox `flush` is remote `accept` only. `lookup` is miss / realloc.
- `Layout` only at the public boundary → `LayoutSpec` inward once.
- No root/shared ownership heap; every run/extent has `HeapId`. Shared `&Heap` = atomics only (`id` / `enqueue` / mode / `has_live`). Active exclusive = `ThreadHeap` + `require_inner` + `AllocatorCtx`. Draining = `Heaps::{enqueue,free,flush}` + `AllocatorCtx`. No `Heap::state()` projection; no `*_fresh` dual alloc APIs.
...
- Next: LTO `vec_many_small` 31.0–32.7. Hit free is `Run::release` (`__rust_dealloc` has no callee-saved). `issued` lives on `RemoteLine`. `Heap::has_live` is `run_live` / `extent_live`. One adopted heap; multi-slot adopt lost on `channel_pipeline`. Do not compact `CLASS_FOR_SIZE`, retry first-fit extent reuse, identity, batch take, O(1) TLS steal, `#135` RSEQ, locate-offset dual free, or multi-slot adopt. Do not port snmalloc.
